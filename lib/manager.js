...
[
    ...
    internals[`is${type}`] = (subj) => subj instanceof klass;
});
...
module.exports = class Manager {
    ...
    async _render(compiled, context, request, callback) {
        ...
        if (this._context) {
            ...
            const isFunction = internals.isFunction(this._context);
            const isAsync = internals.isAsyncFunction(this._context);

            if (isFunction && !isAsync) {

                base = this._context(request);
            }
            ...
            // If base was not an async function but is a promise
            if (base.then) {
