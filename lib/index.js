...
exports.response = function (manager, template, context, options, request) {
    ...
    return new Transponder.Response(source, request, { variety: 'view', marshall: internals.marshall });
};


internals.marshall = function (response, callback) {

    response.source.manager.render(response.source.template, response.source.context, response.source.options, function (err, rendered, config) {

        if (err) {
            return callback(err);
        }

        response._payload = new Transponder.Payload(rendered, response.settings);
