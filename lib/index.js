...
internals.Manager.prototype._loadPartials = function (engine) {
    ...
    var load = function () {
        ...
        var path = internals.path(engine.config.relativeTo, engine.config.partialsPath);
...
internals.Manager.prototype._loadHelpers = function (engine) {
    ...
    var path = internals.path(engine.config.relativeTo, engine.config.helpersPath);
...
internals.Manager.prototype._path = function (template, settings, isLayout) {
    ...
    return (isAbsolutePath ? template : internals.path(settings.relativeTo, (isLayout && settings.layoutPath) || settings.path, template));
};
